...
Read this first: the backend for this feature does not match the workflow originally assumed (Department/Program/Year/Semester/Subject-linked chapters, single-call “upload → generate → done”, HOD approval routing). The real backend is a generic document→video pipeline with a mandatory admin-review gate. Every claim below is drawn from reading `app/edu_video/routes.py`, `schemas.py`, `job_tracker.py`, `pipeline.py`, and `storage.py` in full (repo: `b2b_projectk/`) — not from `VIDEO_GENERATION_PIPELINE.md`, which is confirmed stale on the two points that matter most for this integration (see §5).

> **2026-09-12 update — superseded sections below, kept for history.** The backend has since shipped everything `BACKEND_EDU_VIDEO_CA_HANDOFF.md` asked for, and the frontend now uses it:
> - **Auth is real now** on every `/edu_video/*` route (cookie-based, same as the rest of the app) — the "None" auth column below is out of date. Unauthenticated calls get 401; a job outside your own college 404s.
> - **`department_id` + `chapter_title` are required** on upload for college_admin/faculty/hod (422 without them) and are persisted on the job server-side, not just kept in the browser.
> - **`GET /edu_video/jobs` (list) and `DELETE /edu_video/jobs/{job_id}` (soft-delete) now exist**, scoped to the caller's own college. The frontend's `localStorage` job registry has been **removed entirely** — it was the root cause of jobs "sometimes showing, sometimes not" (a job uploaded in one browser was never visible in another, since `localStorage` is per-browser; this looked like an intermittent bug but was fully deterministic). `src/app/admin/(portal)/content/page.jsx` and the new `src/app/faculty/(portal)/videos/page.jsx` both read `eduVideoApi.listJobs()` as the single source of truth.
> - ~~The review-and-approve workflow is now real, not skipped... "Send to HOD & Faculty"... `POST /jobs/{id}/send_to_teacher`... `AWAITING_TEACHER_REVIEW`...~~ **Superseded again, see the next update below** — this pre-render review design (admin reviews the drafted plan/storyboard before any rendering happens) was replaced with a post-render design per a later product decision.
> - ~~There is still no separate "publish to students" step...~~ **No longer true, see below** — this was built.

> **2026-09-12 update #2 — the actual current design.** Product decision: generation now runs **fully automatically end to end**, and the human review happens **after** a video exists, not before it:
> - **`app/edu_video/pipeline.py::run_pipeline()`** no longer stops at `AWAITING_REVIEW` waiting for a manual `POST .../submit`. The moment the plan is drafted, it immediately continues into rendering in the same background task — `PENDING → EXTRACTING → SCRIPTING → RENDERING → DONE/FAILED` with **no manual gate in between**. `AWAITING_REVIEW`/`AWAITING_TEACHER_REVIEW` still exist as enum values and the legacy `GET`/`PUT .../plan`, `POST .../submit`, `POST .../send_to_teacher` endpoints still work (for any job that somehow lands there), but a poller will essentially never observe those statuses anymore — the transition through them takes milliseconds.
> - **Two new independent timestamps on the job**, not new `status` values: `sent_to_hod_at`/`sent_to_hod_by_user_id` and `published_at`/`published_by_user_id`. New endpoints: **`POST /jobs/{id}/send-to-hod`** (College Admin only valid once `status == DONE` and `output_path` is set — i.e. the video actually finished rendering; 400s otherwise or if already sent) and **`POST /jobs/{id}/publish`** (HOD/Faculty, only valid once `sent_to_hod_at` is set; 400s otherwise or if already published).
> - **The College Admin Content Status table now has 5 columns**: Chapter, Department, **Status** (`Generating`/`Rendering` → `Completed` once rendered → `Awaiting HOD & Faculty approval` once sent → `Published` once the HOD/Faculty approves it, or `Failed`), **Preview** (a real `<video>` player, available the instant `status == DONE`, regardless of send/publish state — see `src/components/shared/VideoPreviewModal.jsx`), and **Action** (`Send to HOD & Faculty` while `Completed`; `Remove` once `Failed` or `Published`).
> - **The Faculty/HOD Video Review page** (`src/app/faculty/(portal)/videos/page.jsx`) now queues by `sent_to_hod_at && !published_at` (not by the legacy `AWAITING_TEACHER_REVIEW` status, which the pipeline barely touches anymore). Each row gets **Preview** + **Publish to students**; once published, it moves to the Published section (Preview + Download).
> - **`published_at` is the closest thing to a real "visible to students" gate that exists today** — but there is still no student-facing consumption endpoint for `edu_video` content anywhere in this codebase (no student portal page reads this table). Building that is a separate, larger piece of work this pass does not include.
> - Migrations: `0026_add_edu_video_tenancy_and_department.py`, `0027_add_edu_video_post_render_workflow.py` — **not yet applied**, same caution as before (see the Testing section at the end of this file).
